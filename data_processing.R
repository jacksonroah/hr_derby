...
generate_sample_hr_data <- function() {
  ...
  if (is.null(roster) || nrow(roster) == 0) {
    return(data.frame(player_name = character(), date = as.Date(character()),
                      team_name = character(), stringsAsFactors = FALSE))
  }

  start_date <- get_opening_day()
  end_date   <- max(Sys.Date(), start_date + 60)  # ensure valid range
  date_range <- seq(start_date, end_date, by = "day")

  sample_data <- data.frame(player_name = character(), date = as.Date(character()),
                             team_name = character(), stringsAsFactors = FALSE)

  power_hitters  <- c("Aaron Judge", "Shohei Ohtani", "Pete Alonso", "Yordan Alvarez",
                      "Mike Trout", "Juan Soto", "Bryce Harper", "Vladimir Guerrero Jr")
  medium_hitters <- c("Ronald Acuna Jr.", "Mookie Betts", "Fernando Tatis Jr.",
                      "Bobby Witt Jr.", "Jose Ramirez", "Gunnar Henderson")

  for (i in seq_len(nrow(roster))) {
    player <- roster$player_name[i]
    team   <- roster$team_name[i]

    hr_count <- if (player %in% power_hitters) {
      sample(15:25, 1)
    ...
    if (length(hr_dates) > 0) {
      sample_data <- rbind(sample_data, data.frame(
        player_name = player, date = hr_dates, team_name = team,
        stringsAsFactors = FALSE
      ))
    }
  }

  sample_data[order(sample_data$date), ]
}
